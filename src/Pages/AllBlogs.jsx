          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            ...
            <div className="relative w-full md:w-96">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-300 dark:border-white/10 text-slate-900 dark:text-white placeholder-slate-500 dark:placeholder-slate-400 focus:outline-none focus:border-purple-500/50 focus:ring-2 focus:ring-purple-500/20 transition-all duration-300"
              />
            </div>
          </div>
        </div>
        ...
        {filteredBlogs.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredBlogs.map((blog, index) => (
              ...
              >
                <div
                  className="bg-slate-50 dark:bg-white/5 backdrop-blur-lg rounded-2xl overflow-hidden border border-slate-200 dark:border-white/10 hover:border-purple-500/30 transition-all duration-300 h-full flex flex-col cursor-pointer"
                ...
                >
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                    ...
                    <h3 className="text-xl font-bold mb-2 text-slate-900 dark:text-white group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors duration-300 line-clamp-2">
                      {blog.title}
                    </h3>
                    ...
                    {blog.tags && blog.tags.length > 0 && (
                      <div className="flex flex-wrap gap-2 mb-4">
                        {blog.tags.slice(0, 4).map((tag, idx) => (
                          <span
                            key={idx}
                            className="px-2 py-1 text-xs rounded-md bg-purple-500/10 text-purple-700 dark:text-purple-300 border border-purple-500/20"
                          >
                            {tag}
                          </span>
                        ))}
